...
from typing import cast, Callable, Dict, Optional, Set, Tuple
...
def _parse_claims(cert: signing_pb2.GovalCert) -> messageClaims:
...
def _get_signing_authority(token: str) -> signing_pb2.GovalSigningAuthority:
    ...
    assert len(token_parts) == 4, 'token is not correctly PASETO-encoded'
    version, purpose, raw_payload, raw_footer = token_parts
    assert version == 'v2'
    assert purpose == 'public'
...
def _verify_raw_claims(
    ...
    claims: Optional[messageClaims] = None,
    ...
    if not any_replid and replid is not None and replid not in claims.repls:
        raise VerificationError('not authorized (replid)')
    if not any_user and user is not None and user not in claims.users:
        raise VerificationError('not authorized (user)')
    if (not any_cluster and cluster is not None
            and cluster not in claims.clusters):
        raise VerificationError('not authorized (cluster)')
...
def _verify_claims(
    ...
    _verify_raw_claims(replid=replid,
                       user=user,
                       cluster=cluster,
                       claims=claims)
...
class Verifier:
    ...
    def verify_chain(
        ...
        if gsa.key_id != '':
            ...
            # verify it.
            return self.verify_token_with_keyid(token, gsa.key_id, gsa.issuer,
                                                pubkey_source), None
...
def verify_identity(
    ...
    v = Verifier()
    raw_repl_identity, _ = v.verify_chain(identity_token, pubkey_source)
    return signing_pb2.GovalReplIdentity.FromString(raw_repl_identity)
...
def verify_ghostwriter(
    ...
    if goval_cert:
        ...
        if not has_ghostwriter_claim:
            raise VerificationError('not authorized (ghostwriter)')
    return goval_token
